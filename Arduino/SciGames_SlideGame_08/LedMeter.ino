...
#define STRIP_LENGTH 32 //20 LEDs on this strip
...
void setLedMeterPotential(int potential, int push){
  //long default_color = 32;
  for(int j=3; j>=0; j--){
    for(int i=STRIP_LENGTH; i>0; i--){
      if(j<1) strip_led[i] = blue;
      else if(j<2) strip_led[i] = blue;
      else if (j<3)strip_led[i] = green;
      else strip_led[i] = red;
      post_frame();
      delay(2);
    }
  }
  for(int x = 0 ; x < potential; x++){
    strip_led[x] = blue;
    post_frame();
    delay(2);
  }
  for(int y=potential; y<STRIP_LENGTH; y++){
    strip_led[y] = off;
    post_frame();
    delay(2); 
  }
...
void setLedMeterGoal(int kinetic, int thermal) {  
  ...
  for(y = x ; y < thermal+x; y++){
    strip_led[y] = red;
  }
  for(int z=y; z<=STRIP_LENGTH; z++){ 
    strip_led[z] = 0; //set rest of LEDs OFF
  }
...
void setLedMeterResult(int kinetic, int thermal) {  
  Serial.print("SET LED METER -- kinetic: ");
  Serial.print(kinetic);
  Serial.print("   thermal: ");
  Serial.println(thermal);
  ...
  int y;
  for(int i=0; i<5; i++){
    for(int i = 0; i< kinetic+thermal; i++){
      strip_led[i] = blue;
    }
    post_frame();
    delay(500);
